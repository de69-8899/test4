...
- **Provider mode switch** via env: `auto | local | external`.
  - `auto` chooses advanced APIs when credentials exist, else local fallback.
- **Shared layers**:
...
See `.env.example` for provider mode and advanced API keys.
